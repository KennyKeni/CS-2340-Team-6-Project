...
import json
from account.forms import CustomUserCreationForm, CustomAuthenticationForm, ProfileUpdateForm
                    ...
                    if user.user_type == UserType.APPLICANT:
                        ...
                        for work_exp in work_experiences:
                            WorkExperience.objects.create(
                                ...
                                end_date=datetime.strptime(work_exp['end_date'], '%Y-%m-%d').date() if work_exp.get('end_date') else None,
            ...
            with transaction.atomic():
                ...
                if updated_user.user_type == UserType.APPLICANT:
                    try:
                        ...
                        if 'headline' in data:
                            applicant.headline = form.cleaned_data.get('headline', applicant.headline)
                        if 'resume' in data:
                            applicant.resume = form.cleaned_data.get('resume', applicant.resume)
                        applicant.save()
                    ...
                    try:
                        ...
                        if 'company' in data:
                            recruiter.company = form.cleaned_data.get('company', recruiter.company)
                        if 'position' in data:
                            recruiter.position = form.cleaned_data.get('position', recruiter.position)
                        recruiter.save()
