...
def applicant_search(request):
    ...
    applicants = Applicant.objects.select_related('account').prefetch_related(
        'work_experiences', 'education', 'skills', 'links'
    ).all()
    ...
    if field_of_study:
        applicants = applicants.filter(education__field_of_study__icontains=field_of_study)
